...
import { Circle, Square, Triangle, Users, FileSearch, Palette, TestTube, Code, CheckCircle2, Rocket, Wrench as WrenchScrewdriver } from 'lucide-react';
...
const ProcessStep: React.FC<ProcessStepProps> = ({ 
  ...
  const getStepDoodles = () => {
    const doodleMap = {
      ...
      "08": [
        <motion.div animate={floatingAnimation} className="text-violet-400" key="support">
          <WrenchScrewdriver className="w-8 h-8" />
        </motion.div>,
    ...
    return doodleMap[number as keyof typeof doodleMap] || [];
  };
    ...
    <div ref={ref} className="relative">
      <motion.div 
        className={`absolute ${align === 'left' ? 'md:left-1/2 left-8' : 'md:left-1/2 left-8'} 
                  -translate-x-1/2 z-20 w-8 h-8 bg-primary-600 rounded-full 
      ...
      <div className={`grid grid-cols-1 md:grid-cols-2 gap-8 items-center ${
        ...
        >
          <div className={`bg-white p-6 rounded-xl shadow-lg transform transition-transform 
            ...
            <h4 className="text-lg font-medium text-primary-700">Step {number}</h4>
